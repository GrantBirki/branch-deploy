...
test('completes a stack deployment created with its checked SHA as the API ref', async () => {
  deployment = {...deploymentRecord(completion), ref: completion.sha}

  const result = await runResultOperation(request)

  assert.equal(result.reasonCode, 'result_completed')
  assert.equal(result.ref, completion.ref)
  assert.equal(result.sha, completion.sha)
  assertCalledTimes(postDeployMock, 1)
})

test('rejects a stack deployment whose API ref matches the checked SHA but whose deployed commit differs', async () => {
  deployment = {
    ...deploymentRecord(completion),
    ref: completion.sha,
    sha: 'f'.repeat(40)
  }
