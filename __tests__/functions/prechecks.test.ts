...
test('recovers inaccessible App metadata on a lower stack PR for an unbound required check', async () => {
  mockNativeStack({
    policies: new Map(
      NATIVE_STACK.pullRequests.map(pull => {
        const policy = stackPolicy({
          headSha: pull.headSha,
          rollup: stackChecksRollup([
            {...STACK_TEST_CHECK, checkSuite: {app: null}}
          ])
        })
        return [
  ...
  loadPrStackRequiredChecksMock.mock.mockImplementation(() =>
    Promise.resolve([{context: 'test1', appId: null}])
  )
  data.inputs.checks = 'required'
...
test('fails closed when lower stack App metadata errors include an unrelated GraphQL error', async () => {
  mockNativeStack({
    policies: new Map([
      [
        121,
        inaccessibleAppError(stackPolicy({headSha: STACK_LOWER_SHA}), [
          {path: INITIAL_QUERY_APP_PATH, type: 'FORBIDDEN'},
          {
            path: ['repository', 'pullRequest', 'reviewDecision'],
            type: 'FORBIDDEN'
          }
        ])
      ]
  ...
  assertCalledTimes(warningMock, 0)
  assertNotCalledWith(infoMock, '✅ stack PR #121 passed deployment checks')
})
